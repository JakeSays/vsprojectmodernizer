...
namespace Std.Tools.Core.Transforms
{
    ...
    {
        private static readonly string[] IgnoreProjectNameValues = { "$(MSBuildProjectName)", "$(ProjectName)" };
        ...
        public PropertySimplificationTransformation(Version targetVisualStudioVersion = null) =>
            _targetVisualStudioVersion = targetVisualStudioVersion ?? new Version(15, 0);

        public void Transform(Project definition)
        {
            var msbuildProjectName = definition.FilePath?.Name;

            msbuildProjectName = !string.IsNullOrEmpty(msbuildProjectName)
                ? Path.GetFileNameWithoutExtension(msbuildProjectName)
            ...
            string msbuildProjectName)
        {
            ...
            foreach (var child in propertyGroup.Elements())
            {
                ...
                var fullState = hasCondition
                    ? ConditionEvaluator.GetConditionState(fullCondition)
                    : null;
                ...
                {
                    if (!hasCondition ||
                        string.IsNullOrWhiteSpace(fullCondition))
                    {
                        return true;
                    }
            ...
            bool ProjectGuidMatchesSolutionProjectGuid() =>
                project.Solution != null && project.Solution.ProjectPaths
